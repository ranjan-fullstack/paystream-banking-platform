package com.paystream.impsservice.outbox;
...
@DisplayName("IMPS OutboxPublisher unit tests")
class OutboxPublisherTest {
    ...
    private OutboxEvent buildEvent(String aggregateId) {
        ...
        e.setAggregateType("ImpsTransaction");
        e.setAggregateId(aggregateId);
        e.setEventType("PaymentInitiated");
        e.setTopic("payment.imps.initiated");
        e.setPayload("{\"paymentMode\":\"IMPS\"}");
    ...
    void testPublishPendingEvents_publishesAndMarksPublished() throws Exception {
        OutboxEvent event = buildEvent("IMPS1234567890001");
    ...
    void testPublishPendingEvents_kafkaFailure_continuesOtherEvents() throws Exception {
        OutboxEvent event1 = buildEvent("IMPS1234567890001");
        OutboxEvent event2 = buildEvent("IMPS1234567890002");
        ...
        when(kafkaTemplate.send(anyString(), eq("IMPS1234567890001"), anyString())).thenReturn(failFuture);
        when(kafkaTemplate.send(anyString(), eq("IMPS1234567890002"), anyString())).thenReturn(successFuture);
        when(outboxEventRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));
