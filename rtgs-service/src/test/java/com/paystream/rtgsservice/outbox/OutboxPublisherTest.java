...
@DisplayName("RTGS OutboxPublisher unit tests")
class OutboxPublisherTest {
    ...
    private OutboxEvent buildEvent(String aggregateId) {
        ...
        e.setAggregateType("RtgsTransaction");
        e.setAggregateId(aggregateId);
        e.setEventType("PaymentInitiated");
        e.setTopic("payment.rtgs.initiated");
        e.setPayload("{\"paymentMode\":\"RTGS\"}");
    ...
    void testPublishPendingEvents_publishesAndMarksPublished() throws Exception {
        OutboxEvent event = buildEvent("RTGS202606290001");
    ...
    void testPublishPendingEvents_kafkaFailure_continuesOtherEvents() throws Exception {
        OutboxEvent event1 = buildEvent("RTGS202606290001");
        OutboxEvent event2 = buildEvent("RTGS202606290002");
        ...
        when(kafkaTemplate.send(anyString(), eq("RTGS202606290001"), anyString())).thenReturn(failFuture);
        when(kafkaTemplate.send(anyString(), eq("RTGS202606290002"), anyString())).thenReturn(successFuture);
        when(outboxEventRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));
